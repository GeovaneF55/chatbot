...
const localizacao = Markup.keyboard([
  Markup.button.locationRequest('Clique aqui para enviar sua localização')
]).oneTime().resize()
...
bot.on('location', async ctx => {
  try {
    ...
    // console.log(lat, lon)
    const res = await axios.get(`${url}?lat=${lat}&lon=${lon}&appid=${env.apiWheatherKey}&units=metric&lang=pt_br`)
